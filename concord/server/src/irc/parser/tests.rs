...
fn test_parse_multiple_colons_in_trailing() {
    let msg = IrcMessage::parse("PRIVMSG #general :time is 12:30:45 and url is http://example.com")
        .unwrap();
    assert_eq!(msg.command, "PRIVMSG");
    assert_eq!(
        msg.params,
...
fn test_parse_prefix_with_full_hostmask() {
    let msg = IrcMessage::parse(":nick!~user@some.host.example.com PRIVMSG #test :hello").unwrap();
...
fn test_server_reply_constructor() {
    let msg = IrcMessage::server_reply("concord", "001", vec!["alice".into(), "Welcome!".into()]);
    assert_eq!(msg.prefix, Some("concord".into()));
    assert_eq!(msg.command, "001");
